...
# ! maybe should / lengths.unsequence(1), but now can equal ori after sum
def duration_loss_sample(logw, logw_, lengths):
    # print(f'logw.shape: {logw.shape}')
    # print(f'logw_.shape: {logw_.shape}')
    # print(f'lengths.shape: {lengths.shape}')
    # input("")
    loss_per_sample = torch.sum((logw - logw_) ** 2, dim=2) / torch.sum(lengths)  # shape: (batch_size,)
